...
/*
Clasic map function
@return the value of x in range of in_min to in_max, remapped to the range of
out_min to out_max
*/
...
/*
Turns the Robot
@param v the magnitude of v is the speed, in percent and the directon of v is
...
/*
Drives the robot
@param driveVec a vector where the magnitude is between 0 and 1, designating the
speed to drive at, and the angle is the angle to drive at
*/
void drive(Comp_Vector driveVec, bool flipDrive) {
  ...
  double x = map(std::abs(driveVec.get_x()), 0,
                 std::abs(100 * cos(drive2.get_angle())), 0, 100);
  double y = map(std::abs(driveVec.get_y()), 0,
                 std::abs(100 * sin(drive2.get_angle())), 0, 100);
  if (x > 100)
    x = 100;
  if (y > 100)
    y = 100;
